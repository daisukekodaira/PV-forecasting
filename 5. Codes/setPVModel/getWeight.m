function weight = getWeight(predictors, forecasted, target)
    ...
    predictors.Time = fix(predictors.Time);
    initHour = min(predictors.Time);
    lastHour = max(predictors.Time);
    ...
    for i = 1:22
        if isnan(data(predictors.Time(i)).forecast)
            % data is not stored yet for the hour
            data(predictors.Time(i)).forecast(1, :) = forecasted(i, :);
            data(predictors.Time(i)).target(1) = target(i);
        else
            % there is stored
            lastStep = size(data(predictors.Time(i)).forecast,1);
            data(predictors.Time(i)).forecast(lastStep+1, :) = forecasted(i, :);
            data(predictors.Time(i)).target(lastStep+1, 1) = target(i);
        end
    end
