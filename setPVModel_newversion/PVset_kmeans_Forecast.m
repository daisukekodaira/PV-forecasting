...
predictorArray = horzcat(TempArray(:,2:4),TempArray(:,7:11));        % Set feature column
...
ForecastData(:,11)=sig_value(6).*result_nb_sunlight + mean_value(6);       % Return standardized data back to real value.
...
Result_pv(:,1) = sig_value(7).*y_pv + mean_value(7);
