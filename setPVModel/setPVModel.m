...
% 2019/02/18 Updated by gyeong gak r3
% 2019/02/22 Updated by Daisuke Kodaira r4.0
% e-mail: gyeong gak: kakkyoung2@gmail.com
...
function flag = setPVModel(LongTermPastData)
    ...
    %% parameters
    ValidDays = 1; % it must be above 1 day. 3days might provide the best performance
    n_valid_data = 96*ValidDays; % 24*4*day
    
    %% Load data
    if strcmp(LongTermPastData, 'NULL') == 0    % if the filename is not null
%         longPast = importdata(LongTermPastData);
    ...
    PVset_kmeans(op_flag, longPast, shortPast, path);
    PVset_fitnet_ANN(op_flag, longPast, shortPast, path);
