function target =PVset_error_correction_1(ForecastData_ANN,result_ForecastData_ANN_mean,shortTermPastData)
...
%% find sunrise,sunset time using mean of shorttermPastData
   ...
   sum_shortterm=zeros(m_forecast,n_shortterm);
    ...
    for i=1:m_forecast
        if mean_shortterm(i,12)==0 && mean_shortterm(i+1,12)~=0      
        sunrise_time=mean_shortterm(i,5:6);
        elseif mean_shortterm(i+1,12)==0 && mean_shortterm(i,12)~=0
        sunset_time=mean_shortterm(i,5:6);
        end
    end
